...
public:
  ...
  static const byte BQ25713addr = 0x6B; // I2C address
...
#define FIELD_RO(data, name, index, size) \
  ...
  struct IBaseRegister : public IBaseReadRegister
  {
    uint16_t set(const uint16_t value)
    {
      ...
      uint16_t constraint = value;
      if (constraint < minVal())
        constraint = minVal();
      else if (constraint > maxVal())
        constraint = maxVal();
      constraint -= minVal();
...
private:
  static bool readDataReg(const byte regAddress, byte* dataVal, const uint8_t arrLen)
  {
    ...
    return ::lampda::hal::i2c::i2c_readData(
                   i2cObjectIndex, BQ25713addr, regAddress, arrLen, dataVal, usesStopBit ? 1 : 0) == 0;
  }

  static bool writeData(const byte regAddress, uint8_t lenght, uint8_t* data)
  {
    // IMPLEMENT THIS FUNCTION
    return ::lampda::hal::i2c::i2c_writeData(
                   i2cObjectIndex, BQ25713addr, regAddress, lenght, data, usesStopBit ? 1 : 0) == 0;
  }
  ...
  static bool writeData16(const byte regAddress, uint16_t val)
  {
    ...
    // IMPLEMENT THIS FUNCTION
    return writeData(regAddress, 2, data);
  }
};
